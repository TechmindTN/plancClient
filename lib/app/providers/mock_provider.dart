...
class MockApiClient {
  ...
  Future<List<User>> getAllUsers() async {
    ...
    if (response.statusCode == 200) {
      return response.data['data'].map<User>((obj) => User.fromFire(obj)).toList();
    } else {
  ...
  Future<List<Category>> getAllCategories() async {
    ...
    int index=0;
    if (response.statusCode == 200) {
      List<Category> categories= response.data['results'].map<Category>((obj) => Category.fromFire(obj)).toList();
      categories.forEach(
        (element) {
  ...
  Future<List<Category>> getAllWithSubCategories() async {
    ...
    if (response.statusCode == 200) {
      return response.data['results'].map<Category>((obj) => Category.fromFire(obj)).toList();
    } else {
      throw new Exception(response.statusMessage);
    }
  }
  ...
  Future<List<Category>> getFeaturedCategories() async {
    ...
    if (response.statusCode == 200) {
      return response.data['data'].map<Category>((obj) => Category.fromFire(obj)).toList();
    } else {
      throw new Exception(response.statusMessage);
    }
  }
